...
class ModeloUsuarios
{
    ...
    static public function mdlMostrarUsuarios($value)
    {
        if ($value != null) {
            ...
            $stmt = Conexion::conectar()->prepare("SELECT u.*, p.activo,p.perfil
                                                    FROM l_usuarios u
    ...
    {
        $stmt = Conexion::conectar()->prepare("DELETE FROM l_re_permisos  WHERE idPerfil = :idPerfil");
    ...
    /* ===================================================
        AGREGAR CONTRASEÑA TEMPORAL A USUARIO
    ===================================================*/
    static public function mdlContraseniaTemporal($cedula, $encriptada)
    {
        $stmt = Conexion::conectar()->prepare("INSERT INTO contrasenias(cedula,PASSWORD) VALUES (:cedula, :contrasenia)");

        $stmt->bindParam(":cedula", $cedula, PDO::PARAM_INT);
        $stmt->bindParam(":contrasenia", $encriptada, PDO::PARAM_STR);
    ...
    /* ===================================================
        CONSULTAR CONTRASEÑA TEMPORAL 
    ===================================================*/
    static public function mdlContraseniaTemporalxId($cedula)
    {
        $stmt = Conexion::conectar()->prepare("SELECT * FROM contrasenias WHERE cedula = :cedula AND TIMESTAMPDIFF(MINUTE, fecha, NOW()) < 5 ORDER BY fecha DESC 
        LIMIT 1 ");
        $stmt->bindParam(":cedula", $cedula, PDO::PARAM_INT);
