...
class ControladorUsuarios
{
	...
	public function ctrIngresoUsuario()
	{
		...
		if (isset($_POST["ingreso_usuario"])) {
			if (
				preg_match('/^[0-9]+$/', $_POST["ingreso_usuario"]) &&
				...
				$contraseniaocasional = ModeloUsuarios::mdlContraseniaTemporalxId($cedulaUsuario);

				if (isset($contraseniaocasional) && $contraseniaocasional != false) {

					$contraseniaocasional = $contraseniaocasional['password'];
				}


				if ($respuesta !== false && $respuesta["Cedula"] == $cedulaUsuario && ($respuesta["Password"] == $encriptar || isset($contraseniaocasional) &&  $_POST["ingreso_contrasenia"] == $contraseniaocasional)) {

					//Ingresó con contraseña ocasional 
					if (isset($contraseniaocasional) &&  $_POST["ingreso_contrasenia"] == $contraseniaocasional) {
						$_SESSION['psswd_ocasional'] = true;
					}
					...
					if ($respuesta['estado'] == 1 && $respuesta['activo'] == 1) {
						...
						========================= */
						$perfil = $_SESSION['permisos'][0]['perfil'];
						$_SESSION['perfil'] = $perfil;


						# Página de inicio
						if (true/* !validarPermiso("M_CONTROL", "R") */) {
							echo '<script>
		...
		if (isset($_POST['ingCambiarPassowrd'])) {
			...
			if (preg_match('/^[0-9]+$/', $_POST["ingCambiarPassowrd"])) {
				...
				if ($usuario !== false && $usuario['Cedula'] != '') {
					...
					$actualizarPassword = ModeloUsuarios::mdlContraseniaTemporal($usuario['Cedula'], $cadena);
					...
					if ($actualizarPassword == "ok") {
						...
						$message = "<h3>Solicitud de cambio de contraseña 🔐</h3><br>
                                    <p>Su contraseña de acceso temporal es: <b>$cadena</b></p>
                                    <p><h4><a href='" . URL_APP . "'>Enlace para ir al sitio web.</a></h4></p>";
	...
	===================================================*/
	static public function ctrCambiarPswdTemporal()
	{
		if (isset($_POST['cambio_nuevacontrasenia'])) {

			$validar = ModeloUsuarios::mdlMostrarUsuarios($_SESSION['cedula']);

			if (is_array($validar)) {

				if ($_POST['cambio_nuevacontrasenia'] == $_POST['confir_nuevacontrasenia']) {
					# validar nueva contraseña, encriptar y guardarla
					$encriptarPassNueva = crypt($_POST['cambio_nuevacontrasenia'], '$2a$07$asxx54ahjppf45sd87a5a4dDDGsystemdev$');
					$encriptarConfirmPass = crypt($_POST['confir_nuevacontrasenia'], '$2a$07$asxx54ahjppf45sd87a5a4dDDGsystemdev$');
					// Volver a validar si coinciden las contraseñas
					if ($encriptarPassNueva == $encriptarConfirmPass) {
						$tabla = "l_usuarios";
						$item1 = "Password";
						$valor1 = $encriptarPassNueva;
						$item2 = "Cedula";
						$valor2 = $validar['Cedula'];
						# Actualizo la contraseña
						$actualizarPassword = ModeloUsuarios::mdlActualizarUsuario($tabla, $item1, $valor1, $item2, $valor2);

						if ($actualizarPassword == 'ok') {

							$_SESSION['psswd_ocasional'] = false;
							echo "<script>
                                    Swal.fire({
                                        icon: 'success',
						...
						echo "<script>
                                    Swal.fire({
                                        ...
                                        text: 'Asegurese de que la nueva contraseña y la confirmación sean iguales.',						
                                        showConfirmButton: true,
                                        confirmButtonText: 'Cerrar',
                                        closeOnConfirm: false								
                                    })
